...
knitr::opts_chunk$set(eval = TRUE, results = FALSE, fig.show = "hide", message = FALSE, warning = F)
```
...
1.  Describe the distribution of responses in this sample. How does it compare to the 
    distribution of responses in the population. **Hint:** Although the `slice_sample` 
...
With this new function, you can repeat this sampling procedure `rep` times in order to build a distribution of a series of sample statistics, which is called the **sampling distribution**. For some [context](https://github.com/tidymodels/infer/issues/337) on the function naming: `slice_sample` took over for an older function `sample_n`.
...
LastName-LastName-L-03.Rmd  (add a third last name if applicable)

LastName-LastName-L-03.html

*Due*: Monday, September 21st (beginning of class time, Eastern time)
