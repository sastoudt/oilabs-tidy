...
```{r view-girls-solution, eval = F}
arbuthnot$girls
```

```{r view-girls-check}
grade_result(
  pass_if(~ identical(as.character(.result[1,1]), "4683"), "You have successfully extracted the counts of girls baptized.")
)
```
...
```{r range-check}
grade_result(
  pass_if(~ identical(as.character(.result[1,1]), "1940"), "You have successfully extracted the counts of girls baptized.")
)
```

```{r range-solution, eval = F}
present %>%
