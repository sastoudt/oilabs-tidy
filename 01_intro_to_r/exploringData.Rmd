...
```{r bsuxldualklhfikm, exercise = TRUE}
____$____
```

```{r bsuxldualklhfikm-solution}
# solution code
arbuthnot$girls
```

```{r bsuxldualklhfikm-check}
# check code
...
```{r uvwytbuptygmgteg, exercise = TRUE}
ggplot(data = ___, aes(x = ___, y = ___)) + 
  geom_point()
```


```{r uvwytbuptygmgteg-solution}
ggplot(data = arbuthnot, aes(x =year, y = boys)) + 
  geom_point()
```

```{r uvwytbuptygmgteg-check}
# check code
gradethis::grade_code()
```
...
```{r dbrizoftmfcrxtaa, exercise = TRUE}
ggplot(data = ___, aes(x = ___, y = ___)) + ___
```

```{r dbrizoftmfcrxtaa-solution}
ggplot(data = arbuthnot, aes(x = year, y = boys)) + 
  geom_line()
```

```{r dbrizoftmfcrxtaa-check}
# check code
gradethis::grade_code()
```
...
```{r gamhocknhqyetwzh-setup}
arbuthnot <- arbuthnot %>% mutate(total = boys + girls) %>%
  mutate(boy_ratio = boys / total)
```


```{r gamhocknhqyetwzh, exercise = TRUE}
ggplot(data = ___, aes(x = ___, y = ___)) + 
  geom_line()
```

```{r gamhocknhqyetwzh-solution}
ggplot(data = arbuthnot, aes(x =year, y = boy_ratio)) + 
  geom_line()
```

```{r gamhocknhqyetwzh-check}
# check code
gradethis::grade_code()
```
...
```{r mnepkpmfycyaglup, exercise = TRUE}
___ %>%
  summarise(min = min(___), max = max(___))
```

```{r mnepkpmfycyaglup-solution}
present %>%
  summarise(min = min(year), max = max(year))
```

```{r mnepkpmfycyaglup-check}
# check code
gradethis::grade_code()
```
...
```{r sugtdrmmiufgrtna, exercise = TRUE}
glimpse(___)
```

```{r sugtdrmmiufgrtna-solution}
# solution code
glimpse(present)
```

```{r sugtdrmmiufgrtna-check}
# check code
gradethis::grade_code()
```
...
```{r xbsmvzydkxccmsuq, exercise = TRUE}
names(___)
```


```{r xbsmvzydkxccmsuq-solution}
names(present)
```

```{r xbsmvzydkxccmsuq-check}
# check code
gradethis::grade_code()
```
...
```{r dvyabiubuhojtxtq, exercise = TRUE}
arbuthnot %>%
  summarise(min = min(boys), max = max(boys))
...
```{r azqsikgawshsgegl, exercise = TRUE}
...
```{r owhverfechvpqajl, exercise = TRUE}
present <- present %>%
  mutate(total = ___)
