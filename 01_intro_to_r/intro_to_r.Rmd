...
These packages will be installed on the server, but later on when you install R and RStudio on your own computer, you will need to install them yourself. You may need to select a server from which to download; any of them will work.
...
We will be using R Markdown to create reproducible lab reports. See the following videos describing why and how. Note they will refer to the `oilabs` package. This has been renamed the `openintro` packge.
...
```{r horst-ggplot, echo=FALSE, results="asis", out.width = "50%"}
...
```{r horst-mutate, echo=FALSE, results="asis", out.width = "50%"}
...
Call me into your breakout room if you are having trouble with this process. Note at the end of your lab report whether or not you both were able to successfully download R and RStudio locally.
...
LastName-LastName-L-01.Rmd  (add a third last name if applicable)

LastName-LastName-L-01.html

*Due*: Monday, September 7th (beginning of class time Eastern time)
