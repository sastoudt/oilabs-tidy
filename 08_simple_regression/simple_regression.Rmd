...
of NA and we don't want those to count towards the calculation of the correlation.
...
the cloud of points. Note, we first need to filter out missing values since `plot_ss` expects complete cases.

```{r plotss-expression-score, eval=FALSE}
# library(tidyverse) ## make sure these are loaded locally
# library(openintro)
# library(statsr)
hfi_nice = hfi %>% filter(!is.na(pf_score), !is.na(pf_expression_control))
plot_ss(x = pf_expression_control, y = pf_score, data = hfi_nice)
```
...
plot_ss(x = pf_expression_control, y = pf_score, data = hfi_nice, showSquares = TRUE)
```
...
1.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    squares that you got? How does it compare to your partner's?
...
that R should look in the `hfi` data frame to find the two variables. Note `lm` can handle missing values, so we can use the full data set here.
...
**Nearly normal residuals**: To check this condition, we can look at a histogram. We'll learn more about the normal distribution in the next lab, but for now, here's what the shape of a normal distribution looks like.

```{r}
sim_norm <- data.frame(sim = rnorm(n = 100000, mean = 0, sd = 1))

ggplot(data = sim_norm, aes(x = sim)) +
...
We can also look at a normal probability plot of the residuals. We'll learn more about this plot in the next lab, but for now we want the points to roughly fall on the red line.
