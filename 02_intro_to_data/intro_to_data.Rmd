...
This lab will occur remotely and in pairs (there may be one group of three). For those of you participating synchronously, you will find the Zoom room information on [Moodle](https://moodle.smith.edu/). I will assign you to a breakout room with your partner(s) from last week. 
...
In RStudio, go to New File -> R Markdown... Then, choose From Template and then choose `Lab Report` (not the specific one we used for Lab 1) from the list of templates. Make sure to name the document appropriately and pick a location for the file where you know how to find it on your computer.

![](img/template-setup.png){width=60%}
...
[Artwork](https://github.com/allisonhorst/stats-illustrations/) by @allison_horst
...
Note: as you complete these exercises remember to give each code chunk a unique name in your lab report.
...
1.  Describe the distribution of the **arrival** delays of these flights using a 
    histogram and appropriate summary statistics. Choose a bandwidth that reveals the features of this distribution **Hint:** The summary 
...
1.  If you were selecting an airport simply based on the on time departure 
...
1. The `dplyr` package is full of [verbs](https://www.youtube.com/watch?v=tcC_Uue7uaQ) that we can use to organize data. Write a question about the flights dataset that could be answered using each of the following functions.
...
1. The `ggplot2` package is full of geometries that we can use to display data. Write a question about the flights dataset that could be answered by a plot that uses each of the following geometries.
...
LastName-LastName-L-02.Rmd  (add a third last name if applicable)

LastName-LastName-L-02.html

*Due*: Monday, September 14th (beginning of class time, Eastern time)
