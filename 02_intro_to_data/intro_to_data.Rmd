...
```{r summary-custom-list-carrier, exercise = T}
sfo_feb_flights %>%
  ___(___) %>%
  summarise(median_dd = ___(arr_delay), iqr_dd = ___(arr_delay))
```
    
```{r summary-custom-list-carrier-solution}
sfo_feb_flights <- nycflights %>%
  filter(dest == "SFO", month == 2)
...
```{r mean-dep-delay-months, exercise = T}
nycflights %>%
  group_by(month) %>%
  summarise(mean_dd = mean(dep_delay)) %>%
...
```{r viz-origin-dep-type, exercise = T}
