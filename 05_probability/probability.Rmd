...
1.  a.) What was Kobe's typical streak length (or mode)? b.) How long was his longest streak of 
    baskets (or maximum)? Make sure to include the accompanying plot in your answer.
...
<div id="boxedtext">
**A note on setting a seed:** Remember that setting a seed will cause R to select the same 
...
```
The number above is completely arbitraty. If you need inspiration, I use the numbers in the date and time (but dropping any zeros). The important thing is that you
...
1.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    `sim_basket`. (this answer will only involve code)
...
1.  Using `calc_streak`, compute the streak lengths of `sim_basket`, and
    save the results in a data frame called `sim_streak`. (this answer will only involve code)

1. a.) What is the typical streak length for this simulated independent shooter with a 45% shooting percentage (in `sim_streak`)? b.) How long is the player's longest streak of baskets in 133 shots? Make sure
    to include a plot in your answer.

1.  a.) How does Kobe Bryant's distribution of streak lengths compare to the 
    distribution of streak lengths for the simulated shooter? I recommend looking at two bar plots to decide. b.) Using this 
    comparison, do you have evidence that the hot hand model fits Kobe's 
    shooting patterns? Explain. Hint: is the "hot hand" associated with longer or shorter streaks than an independent shooter?
...
LastName-LastName-L-05.Rmd  (add a third last name if applicable)

LastName-LastName-L-05.html

*Due*: Monday, October 5th (beginning of class time, Eastern time)
