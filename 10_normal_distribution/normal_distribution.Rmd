...
ggplot(data = dairy_queen, aes(sample = cal_fat)) + 
  geom_qq() + stat_qqline(alpha = 0.7, color = "red", linetype = "dashed")
```
...
from normality leads to deviations of these points from that line.  `stat_qqline()` provides a guide line to help you assess whether points deviate from a line.
...
It would be nice if these were all on the same plot to better judge whether our data fits within the simulated data. We repurpose some of the code that makes up the `qqnormsim` function. It is not crucial that you understand every line of this chunk; just focus on the resulting plot. 
...
ggplot(data = df, aes(sample = x, group = plotnum)) + stat_qq(alpha = 0.6) + stat_qq(data = original, aes(sample = x), col = "red", size = 2)
```
