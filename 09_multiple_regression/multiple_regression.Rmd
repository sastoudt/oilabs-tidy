...
4.  Replot the scatterplot, but this time use `geom_jitter` as your layer to help us with potential overplotting (where multiple points cover one another, but we only see one). What 
...
variables to the model would not add much value to the model. This is because *given* information about one of the variables, the other one doesn't add much new information. In this 
...
15. Drop the variable with the highest p-value and re-fit the model. Did the
...
17. Based on the `m_full` model, describe the characteristics of a professor and 
...
*Due*: Monday, November 2nd (beginning of class time, Eastern time)
