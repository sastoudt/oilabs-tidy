...
Depending on the choice of seed, different people should have gotten a slightly different confidence interval. What proportion of those intervals (across other students working on this lab) would you expect to capture the true population mean? Why?
    
### Exercise 3

Following the example from code chunk `confidence_interval_infer` and data from the one sample you have 
...
b.) Does changing the number of bootstap samples affect the standard error?
...
a.) Change values in the `parameters` code chunk so that you have a sample size of 60, 1000 bootstrap samples for each interval, 50 confidence intervals, and a confidence level of your choice. 

b.) Then run the code in the `run-this1` chunk to determine what proportion of your confidence intervals include the true population proportion. The plot in the `run-this2` chunk can help you visualize the answer, but I'm looking for you to use some `dplyr` verbs to manipulate `toPlot`. **Hint**: You want to `count` each of the two `group`s in `capture_p`. 

c.) Is this proportion exactly equal to the confidence level? If not, explain why. 
